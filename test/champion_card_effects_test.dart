...
void main() {
  testWidgets('attack effect overlay appears only for the attacking card, only during its animation window', (
    tester,
  ) async {
    final attacker = _fireChampion('player-0');

    await tester.pumpWidget(
      _wrap(ChampionCard(champ: attacker, attackNonce: 0, attackerId: null, targetId: null)),
    );
    expect(find.byType(ElementEffectOverlay), findsNothing);

    // Bump the nonce with this champion as the attacker.
    await tester.pumpWidget(
      _wrap(ChampionCard(champ: attacker, attackNonce: 1, attackerId: 'player-0', targetId: 'enemy-0')),
    );
  ...
  testWidgets('a card that is neither attacker nor target never shows the effect overlay', (tester) async {
    final bystander = _fireChampion('player-1');

    await tester.pumpWidget(
      _wrap(ChampionCard(champ: bystander, attackNonce: 1, attackerId: 'player-0', targetId: 'enemy-0')),
    );
    await tester.pump(const Duration(milliseconds: 100));
    expect(find.byType(ElementEffectOverlay), findsNothing);
  });

  testWidgets('the target shows an impact burst (not the cast overlay) while the attacker does not', (tester) async {
